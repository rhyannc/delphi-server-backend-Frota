...
begin
  try
    ...
    if not qryDriver.Eof then
      ...
      raise Exception.Create('Motorista não encontrado.');
    ...
    begin
      ...
      raise Exception.Create('Tarifa não encontrado.');
...
begin
    ...
    raise Exception.Create(' CNPJ já usado.');
...
begin
    try
        ...
        if Trim(name) = '' then
        raise Exception.Create('O nome do Cliente não pode estar vazio.');
        ...
        if qry.Fields[0].AsInteger = 0 then
        raise Exception.Create('Cliente com o ID fornecido não existe.');
...
// Função para inserir dados de Rates (Tarifas)
...
begin
    try
        ...
        // Validação dos parâmetros
        if id_rate <= 0 then
        raise Exception.Create('ID da Tarifa inválido.');
        ...
        if qry.Fields[0].AsInteger = 0 then
        raise Exception.Create('Tarifa com o ID fornecido não existe.');
